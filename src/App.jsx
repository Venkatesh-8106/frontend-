...
export default function App() {
  ...
  if (['admin', 'users', 'requests'].includes(view)) return <div className="min-h-screen bg-paper"><header className="border-b border-[#dce9e4] bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8"><Brand /><div className="flex items-center gap-3"><div className="hidden text-right sm:block"><p className="text-sm font-bold text-ink">{currentUser?.name || 'Administrator'}</p><p className="text-xs text-[#8aa09b]">{currentUser?.role || 'Admin'} workspace</p></div><Button variant="ghost" size="icon" aria-label="Sign out" onClick={handleLogout}><LogOut size={18} /></Button></div></div></header><div className="mx-auto flex max-w-7xl flex-col lg:flex-row lg:gap-8 lg:px-8"><aside className="border-b border-[#dce9e4] bg-white py-3 lg:w-44 lg:shrink-0 lg:border-b-0 lg:border-r lg:bg-transparent lg:py-8"><WorkspaceNav view={view} setView={setView} isAdmin={isAdmin} /></aside><main className="min-w-0 flex-1 px-5 py-8 lg:px-0 lg:py-14">{view === 'users' ? <UserManagement currentUser={currentUser} /> : view === 'admin' ? <AdminDashboard /> : <StudentRequests />}</main></div></div>
