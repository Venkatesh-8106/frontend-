...
export default function App() {
  ...
  if (['admin', 'users', 'requests'].includes(view)) return <div className="min-h-screen bg-paper"><header className="border-b border-[#dce9e4] bg-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 lg:px-8"><Brand /><div className="flex items-center gap-3"><div className="hidden text-right sm:block"><p className="text-sm font-bold text-ink">{currentUser?.name || 'Administrator'}</p><p className="text-xs text-[#8aa09b]">{currentUser?.role || 'Admin'} workspace</p></div><Button variant="ghost" size="icon" aria-label="Sign out" onClick={handleLogout}><LogOut size={18} /></Button></div></div></header><div className="border-b border-[#dce9e4] bg-white"><div className="mx-auto flex max-w-7xl gap-1 overflow-x-auto px-5 lg:px-8"><Button variant={view === 'admin' ? 'primary' : 'ghost'} size="sm" onClick={() => setView('admin')}><BarChart3 size={15} />Overview</Button>{isAdmin && <Button variant={view === 'users' ? 'primary' : 'ghost'} size="sm" onClick={() => setView('users')}><UsersRound size={15} />Users</Button>}{!isAdmin && <Button variant={view === 'requests' ? 'primary' : 'ghost'} size="sm" onClick={() => setView('requests')}><ClipboardCheck size={15} />My requests</Button>}</div></div><main className="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-14">{view === 'users' ? <UserManagement currentUser={currentUser} /> : view === 'admin' ? <AdminDashboard /> : <StudentRequests />}</main></div>
